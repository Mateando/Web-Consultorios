  ...
  <AuthenticatedLayout>
    <div class="py-8 max-w-5xl mx-auto px-4">
      <div class="bg-white rounded shadow p-6">
        <div class="flex justify-between items-center mb-4">
          <h2 class="text-lg font-semibold">Feriados / Días no laborables</h2>
        ...
        <table class="w-full text-sm">
          ...
          <tbody>
            <tr v-for="item in items.data" :key="item.id" class="border-b hover:bg-gray-50">
              ...
              <td class="py-2"><span :class="item.is_recurring ? 'bg-indigo-100 text-indigo-800' : 'bg-gray-100 text-gray-800'" class="px-2 py-0.5 rounded text-xs font-medium">{{ item.is_recurring? 'Sí':'No' }}</span></td>
    ...
    <div v-if="show" class="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
      <div class="bg-white rounded shadow w-full max-w-md p-6">
        <h3 class="text-base font-semibold mb-4">{{ current? 'Editar Feriado':'Nuevo Feriado' }}</h3>
        <form @submit.prevent="save">
          ...
          <div class="mb-4">
            ...
            <input v-model="form.date" type="date" class="w-full border rounded px-3 py-2 text-sm focus:ring focus:border-blue-500" required />
            <p v-if="errors.date" class="text-xs text-red-600 mt-1">{{ errors.date }}</p>
          </div>
          <div class="mb-4 flex items-center gap-3">
            <input id="rec" type="checkbox" v-model="form.is_recurring" />
            <label for="rec" class="text-sm">Repetir cada año</label>
          </div>
          <div class="mb-4">
            <label class="block text-xs font-medium text-gray-600 mb-1">Notas</label>
            <textarea v-model="form.notes" class="w-full border rounded px-3 py-2 text-sm focus:ring focus:border-blue-500"></textarea>
          </div>
...
const form = reactive({ name:'', date:'', is_recurring:false, notes:'' })
const errors = ref({})

function openModal(item=null){
  current.value = item
  form.name = item? item.name: ''
  form.date = item? item.date: ''
  form.is_recurring = item? item.is_recurring: false
...
function save(){
  const url = current.value? route('admin.config.holidays.update', current.value.id): route('admin.config.holidays.store')
  const method = current.value? 'put':'post'
  router[method](url, form, { onError:e=> errors.value=e, onSuccess:()=> close() })
}
function toggle(item){
  router.patch(`/admin/config/holidays/${item.id}/toggle`, {}, { preserveScroll:true })
}
</script>
