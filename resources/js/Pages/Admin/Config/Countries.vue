  ...
  <AuthenticatedLayout>
    <div class="py-8 max-w-6xl mx-auto px-4">
      <div class="bg-white rounded shadow p-6">
        <div class="flex justify-between items-center mb-4">
          <h2 class="text-lg font-semibold">Países</h2>
        ...
        <table class="w-full text-sm">
          ...
          <tbody>
            <tr v-for="item in items.data" :key="item.id" class="border-b hover:bg-gray-50">
              ...
              <td class="py-2">{{ item.iso_code || '-' }}</td>
              ...
              <td class="py-2 text-xs"><Link :href="route('admin.config.provinces', item.id)" class="text-blue-600 hover:underline">Provincias</Link></td>
              <td class="py-2 space-x-2 text-right">
                ...
                <SecondaryButton type="button" @click="toggle(item,'countries')" class="text-gray-600 hover:underline">{{ item.is_active? 'Desactivar':'Activar' }}</SecondaryButton>
              </td>
            </tr>
            <tr v-if="items.data.length===0"><td colspan="5" class="py-4 text-center text-gray-400">Sin registros</td></tr>
          </tbody>
        </table>
    ...
    <div v-if="show" class="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
      <div class="bg-white rounded shadow w-full max-w-md p-6">
        <h3 class="text-base font-semibold mb-4">{{ current? 'Editar País':'Nuevo País' }}</h3>
        <form @submit.prevent="save">
          ...
          <div class="mb-4">
            <label class="block text-xs font-medium text-gray-600 mb-1">Código ISO</label>
            <input v-model="form.iso_code" maxlength="3" class="w-full border rounded px-3 py-2 text-sm focus:ring focus:border-blue-500" />
            <p v-if="errors.iso_code" class="text-xs text-red-600 mt-1">{{ errors.iso_code }}</p>
          </div>
...
const form = reactive({ name:'', iso_code:'' })
const errors = ref({})
function openModal(item=null){ current.value=item; form.name=item? item.name:''; form.iso_code=item? item.iso_code:''; errors.value={}; show.value=true }
function close(){ show.value=false; current.value=null }
function save(){ const url=current.value? route('admin.config.countries.update', current.value.id): route('admin.config.countries.store'); const method=current.value? 'put':'post'; router[method](url, form, { onError:e=>errors.value=e, onSuccess:()=>close() }) }
