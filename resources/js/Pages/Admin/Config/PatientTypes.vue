<template>
  <Head title="Tipos de Paciente" />
  <AuthenticatedLayout>
    <div class="py-8 max-w-5xl mx-auto px-4">
      <div class="bg-white rounded shadow p-6">
        <div class="flex justify-between items-center mb-4">
          <h2 class="text-lg font-semibold">Tipos de Paciente</h2>
        ...
        <table class="w-full text-sm">
          ...
          <tbody>
            <tr v-for="item in items.data" :key="item.id" class="border-b hover:bg-gray-50">
              ...
              <td class="py-2">
                <span :class="item.is_active ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'" class="px-2 py-0.5 rounded text-xs font-medium">{{ item.is_active? 'Activo':'Inactivo' }}</span>
              </td>
              <td class="py-2 space-x-2 text-right">
                ...
                <SecondaryButton type="button" @click="toggle(item,'patient-types')" class="text-gray-600 hover:underline">{{ item.is_active? 'Desactivar':'Activar' }}</SecondaryButton>
              </td>
            </tr>
            <tr v-if="items.data.length===0">
              <td colspan="3" class="py-4 text-center text-gray-400">Sin registros</td>
            </tr>
          </tbody>
        </table>
    ...
    <div v-if="show" class="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
      <div class="bg-white rounded shadow w-full max-w-md p-6">
        <h3 class="text-base font-semibold mb-4">{{ current? 'Editar Tipo':'Nuevo Tipo de Paciente' }}</h3>
...
function save(){
  const url = current.value? route('admin.config.patient-types.update', current.value.id): route('admin.config.patient-types.store')
  const method = current.value? 'put':'post'
  router[method](url, form, { onError:e=> errors.value=e, onSuccess:()=> close() })
}
function toggle(item, segment){
  router.patch(`/admin/config/${segment}/${item.id}/toggle`, {}, { preserveScroll:true })
}
</script>
