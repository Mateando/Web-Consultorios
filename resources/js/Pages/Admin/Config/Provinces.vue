<template>
  <Head :title="'Provincias de ' + country.name" />
  <AuthenticatedLayout>
    <div class="py-8 max-w-6xl mx-auto px-4">
      <div class="mb-4 text-sm"><Link :href="route('admin.config.countries')" class="text-blue-600 hover:underline">&larr; Volver a Países</Link></div>
      <div class="bg-white rounded shadow p-6">
        <div class="flex justify-between items-center mb-4">
          <h2 class="text-lg font-semibold">Provincias de {{ country.name }}</h2>
        ...
        <table class="w-full text-sm">
          ...
          <tbody>
            <tr v-for="item in items.data" :key="item.id" class="border-b hover:bg-gray-50">
              ...
              <td class="py-2 text-xs"><Link :href="route('admin.config.cities', item.id)" class="text-blue-600 hover:underline">Ciudades</Link></td>
              <td class="py-2 space-x-2 text-right">
                ...
                <SecondaryButton type="button" @click="toggle(item,'provinces')" class="text-gray-600 hover:underline">{{ item.is_active? 'Desactivar':'Activar' }}</SecondaryButton>
              </td>
            </tr>
            <tr v-if="items.data.length===0"><td colspan="4" class="py-4 text-center text-gray-400">Sin registros</td></tr>
          </tbody>
        </table>
    ...
    <div v-if="show" class="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
      <div class="bg-white rounded shadow w-full max-w-md p-6">
        <h3 class="text-base font-semibold mb-4">{{ current? 'Editar Provincia':'Nueva Provincia' }}</h3>
...
const props = defineProps({ items:Object, country:Object })
...
function save(){ const url=current.value? route('admin.config.provinces.update', current.value.id): route('admin.config.provinces.store', country.id); const method=current.value? 'put':'post'; router[method](url, form, { onError:e=>errors.value=e, onSuccess:()=>close() }) }
