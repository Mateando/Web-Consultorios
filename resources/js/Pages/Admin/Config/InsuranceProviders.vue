<template>
  <Head title="Obras Sociales / Seguros" />
  <AuthenticatedLayout>
    <div class="py-8 max-w-5xl mx-auto px-4">
      <div class="bg-white rounded shadow p-6">
        <div class="flex justify-between items-center mb-4">
          <h2 class="text-lg font-semibold">Obras Sociales / Aseguradoras</h2>
  <PrimaryButton type="button" @click="openModal()">Nuevo</PrimaryButton>
        ...
        <SecondaryButton type="button" @click="toggle(item,'insurance-providers')" class="text-gray-600 hover:underline">{{ item.is_active? 'Desactivar':'Activar' }}</SecondaryButton>
              </td>
            </tr>
            <tr v-if="items.data.length===0"><td colspan="3" class="text-center py-4 text-gray-400">Sin registros</td></tr>
          </tbody>
        </table>
    ...
    <div v-if="show" class="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
      <div class="bg-white rounded shadow w-full max-w-md p-6">
        <h3 class="text-base font-semibold mb-4">{{ current? 'Editar Proveedor':'Nuevo Proveedor' }}</h3>
...
function save(){ const url=current.value? route('admin.config.insurance-providers.update', current.value.id): route('admin.config.insurance-providers.store'); const method=current.value? 'put':'post'; router[method](url, form, { onError:e=>errors.value=e, onSuccess:()=>close() }) }
