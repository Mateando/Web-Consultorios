<template>
    <Head title="Motivos de Turnos" />
    <AuthenticatedLayout>
        <div class="py-8 max-w-5xl mx-auto px-4">
            <div class="bg-white rounded shadow p-6">
                <div class="flex justify-between items-center mb-4">
                    <h2 class="text-lg font-semibold">Motivos de Turnos</h2>
        ...
        <div v-if="show" class="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
            <div class="bg-white rounded shadow w-full max-w-md p-6">
                <h3 class="text-base font-semibold mb-4">{{ current? 'Editar Motivo':'Nuevo Motivo' }}</h3>
...
// flash handling moved to AuthenticatedLayout.vue (global)
...
function save(){
    const url = current.value? route('admin.config.appointment-reasons.update', current.value.id): route('admin.config.appointment-reasons.store')
    const method = current.value? 'put':'post'
    router[method](url, form, { 
        onError:e=> errors.value=e, 
        onSuccess:() => {
            close()
            const title = method === 'post' ? 'Motivo creado' : 'Motivo actualizado'
            Swal.fire({ toast: true, position: 'top-end', icon: 'success', title, showConfirmButton: false, timer: 2500, timerProgressBar: true })
        }
    })
}
function toggle(item){
    router.patch(`/admin/config/appointment-reasons/${item.id}/toggle`, {}, { preserveScroll:true })
}
</script>
