      ...
      <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg w-full">
        <form @submit.prevent="submitForm">
          <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
            ...
            <!-- Paso 2: Especialidad (opcional si >1) -->
            <div v-show="step===2 && needsSpecialty" class="mb-4">
            ...
            <!-- Paso 3: Estudio (si el doctor tiene) -->
            <div v-show="step===currentStudyStep && showStudySelect" class="mb-4">
              <label class="block text-sm font-medium text-gray-700">Estudio</label>
              <input type="text" v-model="studyQuery" placeholder="Buscar estudio..." class="mb-2 block w-full border-gray-300 rounded-md shadow-sm sm:text-sm" />
              <select v-model="form.study_type_id" class="block w-full border-gray-300 rounded-md shadow-sm">
                <option value="">(Opcional) Seleccionar estudio</option>
...
const filteredDoctorStudyTypes = computed(()=>{ const list= doctor.value?.study_types||[]; if(!studyQuery.value) return list; const q=studyQuery.value.toLowerCase(); return list.filter(s=> s.name.toLowerCase().includes(q)) })
const studyName = computed(()=> (doctor.value?.study_types||[]).find(s=> String(s.id)===String(form.value.study_type_id))?.name || '')

// Steps dynamic
const currentStudyStep = computed(()=> needsSpecialty.value ? 3 : 2)
const dateStep = computed(()=> showStudySelect.value ? (needsSpecialty.value ? 4:3) : (needsSpecialty.value ? 3:2))
...
const nextDisabled = computed(()=>{
  ...
  if(step.value===2 && needsSpecialty.value) return !form.value.specialty_id
  if(step.value===currentStudyStep.value && showStudySelect.value) return false // estudio opcional
  if(step.value===dateStep.value) return !form.value.appointment_date || !form.value.appointment_time
  return false
})
const canSubmit = computed(()=> form.value.patient_id && form.value.appointment_date && form.value.appointment_time && ( !needsSpecialty.value || form.value.specialty_id || doctorSpecialties.value.length===1))
...
const loadSlots = async()=>{
 if(!form.value.doctor_id || !form.value.appointment_date) { availableSlots.value=[]; return }
...
const setupDoctor=()=>{ if(props.initialDoctorId){ form.value.doctor_id=String(props.initialDoctorId); // set specialty automatically if only one
 if(doctorSpecialties.value.length===1){ form.value.specialty_id=doctorSpecialties.value[0].id; // skip specialty step
 if(step.value===1) step.value=2 // advance after patient when appropriate later
 } }
 // Set default date
 if(!form.value.appointment_date){ const now=new Date(); form.value.appointment_date = props.selectedDate || `${now.getFullYear()}-${String(now.getMonth()+1).padStart(2,'0')}-${String(now.getDate()).padStart(2,'0')}` }
}
...
watch(step,(val)=>{ if(val===2 && !needsSpecialty.value){ // skip directly to study/date
 if(showStudySelect.value){ step.value=currentStudyStep.value } else { step.value=dateStep.value } }
})
...
const submitForm = async()=>{
 ...
 try {
  ...
  Swal.fire({ icon:'success', title:'Cita creada', timer:1800, showConfirmButton:false })
  emit('saved')
