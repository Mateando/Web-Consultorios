...
// Filter doctors by pre-selected specialty
...
// Actually use doctorFilteredList
...
// Steps
const studyStep = 3
const dateStep = computed(()=> showStudySelect.value ? 4 : 3)
...
const nextDisabled = computed(()=>{
 ...
 if(step.value===studyStep && showStudySelect.value) return false
 if(step.value===dateStep.value) return !form.value.appointment_date || !form.value.appointment_time
 return false
})
const canSubmit = computed(()=> form.value.patient_id && form.value.doctor_id && form.value.appointment_date && form.value.appointment_time )
...
watch(()=>props.show,(v)=>{ if(v){ resetForm(); loadReasons(); if(props.initialSpecialtyId){ form.value.specialty_id=String(props.initialSpecialtyId) } setupDate() } })
