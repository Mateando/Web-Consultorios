...
echo "Verificando permisos de edición como usuario 'administrador' para una cita futura...\n";
...
// Buscar una cita FUTURA (strictly future)
$appointment = Appointment::whereNotIn('status', ['cancelada'])->where('appointment_date', '>', Carbon::now())->orderBy('appointment_date', 'asc')->first();
if (!$appointment) {
    echo "No se encontró ninguna cita futura para probar. Puedes crear una cita de prueba o ejecutar seeders.\n";
    exit(1);
}
...
try {
    ...
    echo "Si no hubo excepción, la operación pasó las verificaciones de permisos.\n";
